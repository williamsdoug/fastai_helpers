...
def show_results(results, key=None, show_details=True, limit=None, sort_param='error_rate'):
    ...
    if len(results) > 1:
        ...
        print(f'Error -- best: {np.min(err):.3f}  med: {np.median(err):.3f}   Loss -- best: {np.min(loss):.3f}  med: {np.median(loss):.3f} -- {key}')
    ...
    for key, stats in results[:limit]:
        print(f"error_rate: {stats['error_rate']:.3f}   accuracy: {stats['accuracy']:.3f}   loss:{stats['loss']:.4f} -- {key}")
